import pytest
import asyncio
from app.services import fflow_service
...
async def test_create_flow_success(db):
    ...
    created_flow = await fflow_service.create_flow(db, name, desc)
...
async def test_get_flow_success(db):
    ...
    created_flow = await fflow_service.create_flow(db, name, desc)
    found_flow = await fflow_service.get_flow(db, str(created_flow.flowId))
...
async def test_get_flows_success(db):
    await fflow_service.create_flow(db, "Fluxo 1", "Desc 1")
    await fflow_service.create_flow(db, "Fluxo 2", "Desc 2")

    all_flows = await fflow_service.get_flows(db)
...
async def test_update_flow_success(db):
    original_flow = await fflow_service.create_flow(db, "Nome Original", "Desc Original")

    await asyncio.sleep(0.25)

    await fflow_service.update_flow_metadata(
        db,
    ...
    updated_flow = await fflow_service.get_flow(db, str(original_flow.flowId))
...
async def test_delete_flow_success(db):
    flow_to_delete = await fflow_service.create_flow(db, "Fluxo para Deletar", "...")
    flow_id_str = str(flow_to_delete.flowId)

    await fflow_service.delete_flow(db, flow_id_str)

    with pytest.raises(NotFoundException):
        await fflow_service.get_flow(db, flow_id_str)
