<template>
  <div class="space-y-4">
    <div class="space-y-3">
      ...
      <div v-for="(variable, index) in variables" :key="`variable-${index}`"
        class="rounded-lg variables-wrapper p-4 relative">
        <div class="flex-1 flex flex-col gap-4">
          <div class="w-full">
            <label :for="`name-${index}-${variable.displayName}`"
              class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Nome da Variável
            </label>
            <InputText v-model="variable.displayName" :id="`name-${index}-${variable.displayName}`"
              placeholder="Ex.: full_name" class="w-full !bg-transparent" @input="updateVariables" />
          </div>

          <div class="w-full">
            <label :for="`type-${index}-${variable.displayName}`"
              class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Tipo
            </label>
            <Select v-model="variable.type" :id="`type-${index}-${variable.displayName}`" :options="variableTypes"
              option-label="label" option-value="value" placeholder="Selecione o tipo" class="w-full !bg-transparent"
              @change="updateVariables" />
          </div>

          <div class="flex items-center w-full">
            <div class="flex items-center gap-2">
              <Checkbox v-model="variable.required" :id="`required-${index}-${variable.displayName}`" binary
                @change="updateVariables" />
              <label :for="`required-${index}-${variable.displayName}`"
        ...
        <Button icon="pi pi-times" class="!absolute top-0 right-0" severity="secondary" size="small" text rounded
          @click="removeVariable(index)" />
      </div>
    </div>

    <Button icon="pi pi-plus" label="Adicionar variável" severity="secondary" outlined size="small"
      @click="addVariable" />
